...
class User::RegisterableTest < ActiveSupport::TestCase
  ...
  test "it creates some assistants with tools enabled and some without" do
    people(:ali_invited).update!(personable_type: "User", personable_attributes: { first_name: "John", last_name: "Doe", })
    assert people(:ali_invited).user.assistants.where(supports_tools: false).present?
    assert people(:ali_invited).user.assistants.where(supports_tools: true).present?
  end
end
