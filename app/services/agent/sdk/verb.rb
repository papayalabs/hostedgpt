module Agent
  class SDK::Verb
    ...
    def possible_test_warning(method)
      if Rails.env.test?
        Rails.logger.warn "SDK::#{method.to_s.capitalize} is making a real network request in test env. Define a mock: self.mocked_response_#{method}_#{@calling_method}" unless ENV["SDK_DOES_NOT_WARN"]
      end
    end
