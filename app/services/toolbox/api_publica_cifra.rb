  ...
  # ---------------------------------------------------------------------------
    ...
    describe :exporta_a3, <<~S
      Exporta los datos de gestión económica en formato archivo compatible con A3.
      Parámetros:
        - fecha_from: fecha inicio en formato YYYY-MM-DD
        - fecha_to: fecha fin en formato YYYY-MM-DD
        - empresa_id_i: identificador de la empresa (integer)
      Devuelve un hash con la clave :url que contiene la URL del archivo exportado.
    S

    def exporta_a3(fecha_from_s: nil, fecha_to_s: nil , empresa_id_i: nil)
      params = {
        fecha_from: fecha_from_s,
        fecha_to: fecha_to_s,
        empresa_id: empresa_id_i
      }
      response = get(url("/api_publica/v1/gestion_economica/exporta_a3"), token_vigente).param(params)
      file_url = response.try(:url) || response.try(:[], :url) || response.try(:[], "url")
      return { url: file_url } if file_url.present?
      { error: true, message_to_user: "No se pudo exportar el archivo A3" }
    end
