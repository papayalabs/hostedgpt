  ...
  included do
    ...
    scope :for_conversation_version, ->(version) do
      ...
      if version.to_s == "latest"
        ...
        last_msg_for_version = Agent::Message.select(:index, :version).where(conversation_id: c_id).
          order(version: :desc).order(index: :desc).first
      else
        last_msg_for_version = Agent::Message.select(:index, :version).where(conversation_id: c_id).where("version <= ?", version).
          order(version: :desc).order(index: :desc).first
      end
